...
class _DetailScreenState extends State<DetailScreen> {
  ...
  Widget build(BuildContext context) {
    List bible = [];
    int bibleNum = 0;
    List selectedB = Provider.of<AppStateNotifier>(context, listen: false)
        .getSelectedBibleState();
    if (selectedB[0] == true) {
      bible.add(korhkjv);
      bibleNum++;
    }
    ...
    final chapterList =
        Iterable<int>.generate(bible[0][widget.book].length).toList();
                                ...
                                children: korOldMidB.mapIndex((val, idx) {
                                  return MaterialButton(
                                      onPressed: () {
                                        ...
                                        if (verseHistory.length - 1 >
                                            widget.idx)
                                          verseHistory.removeRange(
                                              widget.idx + 1,
                                              verseHistory
                                                  .length); //이후 앞으로가기 기록 삭제
                                        ...
                                        Navigator.push(
                                            ...
                                            MaterialPageRoute(
                                                builder: (context) =>
                                                    DetailScreen(
                                                        name: korOldShortB[idx],
                                                        book: idx,
                                                        chapter: 0,
                                                        verse: 0,
                                                        idx: widget.idx + 1)));
                                      },
                                      color: widget.book == idx
                                          ? Color(0xff0321ab)
                                          : isDark
                                              ? AppTheme.darkMode.focusColor
                                              : AppTheme.lightMode.focusColor,
                                      minWidth: 0,
                                      height: 0,
                                      ...
                                      onPressed: () {
                                        ...
                                        Navigator.push(
                                            ...
                                            MaterialPageRoute(
                                                builder: (context) =>
                                                    DetailScreen(
                                                        name: korNewShortB[idx],
                                                        book: idx + 39,
                                                        chapter: 0,
                                                        verse: 0,
                                                        idx: widget.idx + 1)));
                                      },
                                      color: widget.book == idx + 39
                                          ? Color(0xff0321ab)
                                          : isDark
                                              ? AppTheme.darkMode.focusColor
                                              : AppTheme.lightMode.focusColor,
    ...
    void _showVerseDialog() {
      showDialog(
          context: context,
          builder: (BuildContext context) {
            final verseList = Iterable<int>.generate(
                    bible[0][widget.book][currentPage].length)
                .toList();
            return AlertDialog(
                content: Container(
              width: 300,
          ...
          onHorizontalDragEnd: (DragEndDetails details) {
            if (details.velocity.pixelsPerSecond.dx > 0 && currentPage > 0) {
              setState(() {
                ...
                currentPage < bible[0][widget.book].length - 1) {
              setState(() {
                currentPage += 1;
                _scrollController?.jumpTo(index: 0);
                verseHistory[widget.idx].chapter = currentPage;
...
class _VerseListState extends State<VerseList> {
  ...
  Widget build(BuildContext context) {
    bool isDark =
        Provider.of<AppStateNotifier>(context, listen: false).getModeState();
    double _fontSize = Provider.of<AppStateNotifier>(context, listen: false)
        .getFontSizeState();
                          ...
                          title: Text(
                            ...
                            style: TextStyle(
                                ...
                                color: widget.selected[i][j]
                                    ? selectedColors[j]
                                    : unSelectedColors[j],
                                backgroundColor: highlights[widget.book]
                                        [widget.chapter][i]
                                    ? highlightColor
                                    : null),
                          ),
